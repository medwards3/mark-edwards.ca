...
def get_json():
	...
	feed = urllib.request.urlopen("https://www.googleapis.com/calendar/v3/calendars/41lp9k24ggd5e2u5no7tmpldlg@group.calendar.google.com/events?singleEvents=true&orderBy=startTime&timeMin={}&key=AIzaSyDeEyzvi8F9s3_442dfQ8FC-IYdDa7S3eU".format(google_date)).read()
	return feed

def get_past_json():
	google_date = datetime.now().strftime("%Y-%m-%dT%H:%M:%S.000z")
	feed = urllib.request.urlopen("https://www.googleapis.com/calendar/v3/calendars/41lp9k24ggd5e2u5no7tmpldlg@group.calendar.google.com/events?singleEvents=true&orderBy=startTime&timeMax={}&key=AIzaSyDeEyzvi8F9s3_442dfQ8FC-IYdDa7S3eU".format(google_date)).read()
	return feed
